  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
      ...
      if margin > 0:
        loss += margin
        dW[:, j] += X[i]
        num_margin_classes += 1

    dW[:, y[i]] += - num_margin_classes * X[i]
...
def svm_loss_vectorized_(W, X, y, reg):
  """
  ...
  #############################################################################
  scores = X.dot(W)
  corr_class_scores = scores[np.arange(len(scores)), y].reshape(len(scores), 1)
  margins = scores - corr_class_scores + 1
  margins[np.arange(len(margins)), y] = 0.
  margins[margins < 0.] = 0.



  loss = np.sum(margins) / len(margins)
  loss += reg * np.sum (W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
